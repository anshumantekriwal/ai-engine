...
Analysis: Event-driven. Multiple concurrent positions allowed — track with array (including size per trade for sandboxed closes). Trailing stop for exit. Liquidation side "B" = long liquidated (buy dip), "A" = short liquidated (short spike). Total tracked size matters for partial closes.
...
- **Skip events also communicated: user sees WHY a liquidation wasn't acted on**
</examples>
