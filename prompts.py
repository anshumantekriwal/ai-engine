...
## 8. Verbose Logging Requirements

**Every decision the agent makes MUST be logged with clear reasoning.** This is critical for debugging
and monitoring. Logging flows to Supabase via `updateState()` and `console.log()`.

Rules:
- **Every trade execution**: Log coin, side, size, price, notional, leverage, trigger reason, and any
  calculated values (SL/TP prices, indicator values) that led to the decision.
- **Every skipped signal**: Log WHY the signal was skipped. Examples:
  - "BTC: Skipped buy — trade idea already active (entered at $95000, RSI was 28)"
  - "ETH: Skipped entry — position too small ($8.50 notional, min $10)"
  - "SOL: Skipped entry — safety limits blocked (reason: daily loss limit reached)"
- **Every exit**: Log the exit reason, entry price, exit price, PnL estimate, and hold duration.
- **Every state change**: Log what changed and why.
- **Position checks**: When checking positions, log what was found.
- **Indicator values**: When evaluating indicators, log the current values and thresholds.

Use `console.log()` for detailed operational logs and `await this.updateState()` for key state
transitions that should be visible in the dashboard. The `message` parameter of updateState should
be a human-readable summary of what happened and why — NOT just "order executed" but
"BTC LONG: 0.001 BTC @ $95,230 (5x leverage, RSI=28.5 triggered buy, SL=$93,417 TP=$97,148)".

**Do NOT use empty or generic log messages.** Every log must answer: WHAT happened, to WHICH coin,
and WHY.
</strategy_guide>
...
2. **Log trigger received**: What fired and with what values.
3. **Detect external closes**: Check if positions flagged as active still exist. Reset stale state.
...
6. **Check trade-idea state**: Is this a new idea or a stale repeat? If skipped, log WHY.
...
10. Wrap everything in try-catch. Log errors with full context.
...
10. Plan verbose logging: every decision must log WHAT, WHICH coin, and WHY.
...
<examples>
### Example 1: RSI Mean Reversion (dollar sizing, technical triggers, ROI-based risk)
...
Analysis: Mean reversion. $50 notional → size = $50 / currentPrice. ROI-based SL/TP: price move = ROI% / leverage. User explicitly wants opposite-close. Trade idea = one RSI excursion; reset on opposite signal or external close.
...
- Closes opposites ONLY because the strategy says to
...
- Every `updateState` message is a natural-language sentence explaining what happened and why

### Example 2: Multi-coin EMA Crossover (scheduled trigger, manual indicators, "including leverage" sizing)
...
Analysis: Trend-following crossover. "$10 including leverage" means $10 notional (total position value), NOT margin. EMA crossover needs scheduled trigger + manual calculation (technical triggers can't compare two indicators). Process all coins for exits first, then entries.
...
### Example 3: Liquidation Scalping (event triggers, concurrent positions, trailing stops)
...
Analysis: Event-driven. Multiple concurrent positions allowed — track with array. Trailing stop for exit. Liquidation side "B" = long liquidated (buy dip), "A" = short liquidated (short spike).
...
- Concurrent positions: tracked in an array, gated by `maxConcurrentPositions`
- Trailing stop: `placeTrailingStop(coin, !isBuy, size, trailPercent, true)`
- Scheduled cleanup checks if positions still exist and clears stale tracking
- Small liquidations below threshold are ignored silently (no log spam)
</examples>

<communication>
`updateState` messages are the agent's ONLY way to communicate with the user. Write them as natural-language sentences that build trust:
- Explain WHAT happened: "Opened LONG — 0.0005 BTC @ $94,200 ($50 notional, 10x leverage)"
- Explain WHY: "RSI dropped to 28.4, triggering buy signal"
- Explain WHY NOT (when skipping): "RSI still oversold (29.1) but already have a position from this excursion. Waiting for reset."
- Include numbers: prices, sizes, notional values, fees, PnL estimates
- On errors: "Order failed — insufficient margin. Will retry on next signal."
Do NOT write generic messages like "trade executed" or "signal received".
</communication>
...
- Missing `updateState()` at key decision points
