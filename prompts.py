...
## SL/TP Fill Detection (IMPORTANT)

The system automatically detects when stop-loss, take-profit, or trailing-stop orders fill on the
exchange. When detected, it logs the trade to the database (with actual fill price and PnL),
notifies the user via updateState, and syncs positions. **You do NOT need to manually call
logTrade or syncPositions for SL/TP fills.**

However, you MUST register your SL/TP orders so the system knows which orders to watch:

this.registerSlTpOrders(coin, {{ sl: slResult, tp: tpResult }})
/**
 * Register pending SL/TP orders for automatic fill detection.
 * Call this IMMEDIATELY after placing stop-loss and/or take-profit orders.
 *
 * @param {{string}} coin - Coin symbol (e.g. "BTC")
 * @param {{Object}} orders - {{ sl: OrderResult, tp: OrderResult, trailing: OrderResult }}
 *   Each field is optional. Only orders with success=true are registered.
 *
 * @example
 ...
 * Call this BEFORE manually closing a position and cancelling its SL/TP orders.
 * This prevents the system from double-processing a fill that you're already handling.
 *
...
**External close detection:** SL/TP orders placed on the exchange fill asynchronously. The system
automatically detects these fills (if you called `registerSlTpOrders`), logs the trade with
the actual fill price and PnL, and notifies the user. You do NOT need to call `logTrade` or
`syncPositions` for SL/TP fills — they are handled automatically.

However, your generated code still needs to detect that the position is gone and reset
`tradeState` so the strategy doesn't get stuck. Verify the position still exists:
   ```javascript
...
if (this.tradeState[coin].ideaActive && !positionExists) {{
  ...
  // NOTE: logTrade and syncPositions are already handled by the SL/TP fill detection system
...
**After placing SL/TP orders, ALWAYS call `this.registerSlTpOrders(coin, {{ sl: slResult, tp: tpResult }})`**
so the system can automatically detect when they fill, log the trade with the actual fill price, and
notify the user. If you manually close a position and cancel its SL/TP, call `this.clearSlTpOrders(coin)`
first to prevent double-processing.
...
**Leverage in PnL, Drawdown, and Profit Calculations (CRITICAL — READ CAREFULLY):**

On a leveraged perpetual futures position, a small price change produces an AMPLIFIED effect on the
trader's margin. ALL percentage-based thresholds (drawdown, profit targets, loss limits) that refer
to the position or the trader's investment MUST account for leverage.

Core formulas:
- `unrealizedPnl = (currentPrice - entryPrice) * size` (for longs; reverse for shorts)
- `pnlPercent_ROI = (unrealizedPnl / margin) * 100`, where `margin = notional / leverage`
- Equivalently: `pnlPercent_ROI = priceChangePercent * leverage`

**When the user says "position is down 10%", they mean the ROI on their margin is -10%.** With 10x
leverage, that's only a 1% price move. With 50x leverage, it's a 0.2% price move.

**Decision tree for percentage thresholds:**

| User says | What it means | Formula |
|---|---|---|
| "down X%" or "X% drawdown" (no qualifier) | ROI on margin = -X% | `priceThreshold = entryPrice * (1 - X/100/leverage)` (long) |
| "profit X%" or "up X%" (no qualifier) | ROI on margin = +X% | `priceThreshold = entryPrice * (1 + X/100/leverage)` (long) |
| "X% price move" (explicit) | Raw price change = X% | `priceThreshold = entryPrice * (1 ± X/100)` |

**For dollar PnL thresholds** (e.g., "profit above $0.01 after fees"):
- `unrealizedPnl = (currentPrice - entryPrice) * size` — this already reflects leverage implicitly
  because `size = notional / price` and `notional = margin * leverage`.
- Compare `unrealizedPnl - fees` against the dollar threshold directly.

**Example — "$10.50 position at max leverage, add when down 10%":**
If max leverage = 50x, margin = $10.50/50 = $0.21, notional = $10.50.
"Down 10%" means ROI = -10%, so: `priceMove = 10% / 50 = 0.2%`.
At entry $100: threshold = $100 * (1 - 0.002) = $99.80 (long).
NOT $100 * (1 - 0.10) = $90.00 — that would be a 10% price move, which is 500% ROI loss.

**Always convert user percentages to price levels using `priceChange = percentage / leverage`
unless the user explicitly says "price move" or "not factoring leverage".**
...
8b. **Register SL/TP orders**: After placing SL/TP, call `this.registerSlTpOrders(coin, {{ sl: slResult, tp: tpResult }})`.
...
- **Leverage in thresholds**: When computing drawdown %, profit %, or any percentage-based condition, ALWAYS divide by leverage to convert from ROI% to price-move%. A "10% drawdown" at 50x leverage = 0.2% price move. NEVER compare raw price change % against a user's percentage threshold without accounting for leverage.
</execution_guide>
...
7b. **For ANY percentage threshold (drawdown, profit, add-on, scale-out): convert to price-move using `% / leverage`. Verify this is done — it is the #1 leverage mistake.**
...
- **`registerSlTpOrders(coin, {{ sl, tp }})` after placing SL/TP — enables automatic fill detection**
- **`clearSlTpOrders(coin)` before manual close and cancel — prevents double-processing**
...
- External close detection resets tradeState when SL/TP fills on exchange (logging is automatic)
...
- Trailing stop: `placeTrailingStop(coin, !isBuy, size, trailPercent, true)` + `registerSlTpOrders(coin, {{ trailing: result }})`
- **`registerSlTpOrders` enables automatic fill detection for trailing stops too**
...
- **LEVERAGE IN THRESHOLDS: Any percentage-based condition (drawdown %, profit %, add-on %, scale-out %) that compares raw price change without dividing by leverage. E.g., "down 10%" at 50x leverage should trigger at 0.2% price move, NOT 10% price move. This is a critical error that causes wildly wrong behavior.**
...
- **SL/TP TRACKING: Missing `registerSlTpOrders()` after placing stop-loss/take-profit/trailing-stop orders — system won't auto-detect fills**
- **SL/TP TRACKING: Missing `clearSlTpOrders()` before manually closing a position and cancelling SL/TP — risks double-processing**
